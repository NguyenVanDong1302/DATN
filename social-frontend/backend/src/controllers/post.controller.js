...
function serializePost(post, userId, commentsCount = 0) {
  ...
  const firstImageUrl = imageUrls[0] || obj.imageUrl || "";
...
async function listPosts(req, res, next) {
  try {
    ...
    if (requestedVisibility && visibilityEnum.includes(requestedVisibility)) {
      ...
      if (requestedVisibility === "private" && viewerId) {
        filters.authorId = viewerId;
      }
      if (requestedVisibility === "private" && !viewerId) {
        filters._id = null;
      }
    ...
    const [items, total] = await Promise.all([
      Post.find(filters).sort({ createdAt: -1, _id: -1 }).skip(skip).limit(limit),
