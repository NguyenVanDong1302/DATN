...
export default function MessagesPage() {
    ...
    ;(async () => {
      try {
        ...
        setConversations((prev) =>
          prev.map((item) => (item.id === activeId ? { ...item, ...detail, unreadCount: 0, nickname: detailSettings.nickname, isBlocked: detailSettings.isBlocked } : item)),
        )
  ...
  useEffect(() => {
    ...
    const onMessageNew = (message: ChatMessage) => {
      setMessagesByConversation((prev) => {
        ...
        const optimisticIndex = arr.findIndex((item) => item.optimistic && item.senderUsername === message.senderUsername && item.text === message.text)
  ...
  const handleSend = async () => {
    ...
    if (!value || !activeId || sending || isBlocked) return
    setSending(true)
    setError('')
    ...
    const optimisticMessage: ChatMessage = {
      ...
      senderId: 'me',
      senderUsername: state.username || 'me',
    ...
    setMessagesByConversation((prev) => ({ ...prev, [activeId]: [...(prev[activeId] || []), optimisticMessage] }))
    setConversations((prev) => prev.map((item) => (item.id === activeId ? { ...item, lastMessageText: value, lastMessageAt: optimisticMessage.createdAt } : item)))
    setText('')
    ...
    try {
      ...
      if (!ack?.ok) {
        ...
        setMessagesByConversation((prev) => ({
          ...prev,
          [activeId]: (prev[activeId] || []).map((item) => (item.id === optimisticId ? fallback : item)),
        }))
      }
    } catch (err: any) {
      setMessagesByConversation((prev) => ({
        ...prev,
        [activeId]: (prev[activeId] || []).filter((item) => item.id !== optimisticId),
      }))
    ...
    <div className="ig-msg">
      <div className={`ig-msg__wrap ${isDetailOpen ? 'is-detail-open' : ''}`}>
        <aside className="ig-msg__left">
          ...
          <div className="ig-msg__list" ref={listRef}>
              ...
              <button key={item.id} type="button" className={`ig-msg__item ${item.id === activeId ? 'is-selected' : ''}`} onClick={() => setActiveId(item.id)}>
                ...
                <div className="ig-msg__itemMid">
                  <div className="ig-msg__itemTitle">
                    <span className="ig-msg__itemName">{item.nickname?.trim() || item.peer.username}</span>
                  </div>
              ...
              <div className="ig-msg__mainShell">
                    ...
                    <div className="ig-msg__body">
                      ...
                      {activeMessages.map((message, index) => {
                        ...
                        return (
                          ...
                          <div className={`ig-msg__row ${fromMe ? 'is-me' : 'is-them'}`}>
                  ...
                  <div className="ig-msg__composerWrap">
                    ...
                    <div className="ig-msg__composer">
                      ...
                      <div className="ig-msg__composerActions">
                        <button className="ig-msg__iconBtn" type="button">🎤</button>
                        <button className="ig-msg__iconBtn" type="button">🖼</button>
                        <button className="ig-msg__iconBtn" type="button" onClick={() => void handleSend()} disabled={sending || !text.trim() || isBlocked}>
                          {sending ? '...' : '➤'}
                        </button>
                      </div>
                    </div>
                ...
                {isDetailOpen ? (
                  <aside className="ig-msg__detail">
                    ...
                    <button type="button" className="ig-msg__detailAction" onClick={() => void toggleBlock()} disabled={savingDetail}>
                      {isBlocked ? 'Bỏ chặn người dùng' : 'Chặn người dùng'}
                    </button>
                    <button type="button" className="ig-msg__detailDanger" onClick={() => void clearHistory()} disabled={savingDetail}>
