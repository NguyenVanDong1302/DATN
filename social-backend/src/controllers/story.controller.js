...
const { notifyStoryLike } = require('../services/notification.service');
...
function serializeStory(story, viewerId = '') {
  return {
    ...
    thumbnailUrl: normalizePublicMediaUrl(story.thumbnailUrl),
...
async function resolveViewer(req) {
  ...
  const viewer = await User.findOne({ username: rawUsername }).select('_id username avatarUrl');
...
async function createStory(req, res, next) {
  try {
    ...
    const story = await Story.create({
      ...
      mediaUrl: normalizePublicMediaUrl(`/uploads/posts/stories/${path.basename(file.path)}`),
      thumbnailUrl: isVideo ? await buildVideoThumbnail(file.path, path.basename(file.path)) : normalizePublicMediaUrl(`/uploads/posts/stories/${path.basename(file.path)}`),
...
async function toggleStoryLike(req, res, next) {
  try {
    ...
    await story.save();
    if (liked) {
      await notifyStoryLike({ story, actorId: String(viewer._id), actorUsername: viewer.username });
    }
    res.json({ ok: true, data: { liked, likesCount: story.likes.length } });
  } catch (err) { next(err); }
}

module.exports = { listStories, createStory, toggleStoryLike };
